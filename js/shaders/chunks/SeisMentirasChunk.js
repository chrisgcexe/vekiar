...
// MÁSCARA PINTADA (Herramienta de Edición)
float paintedMask = texture2D(tCustomMask, vGlobalPos).r;

// Modo Debug Visual: Si estamos pintando la máscara, la mostramos con colores de depuración
if (uIsPaintingMask > 0.5) {
    vec3 mathColor = vec3(0.0, 0.5, 1.0); // Azul tenue para la forma base matemática
    vec3 paintColor = vec3(1.0, 0.0, 0.0); // Rojo fuerte para lo que tú pintas
    
    vec3 debugColor = mix(mathColor, paintColor, paintedMask);
    float debugAlpha = clamp(mathMask * 0.3 + paintedMask * 0.8, 0.0, 1.0);
    
    if (debugAlpha > 0.0) {
        gl_FragColor.rgb = mix(gl_FragColor.rgb, debugColor, debugAlpha);
    }
    return; // Cortamos el shader aquí para no dibujar el humo y ver claramente
}
...
if (maskMentiras > 0.0) {
    // 1. Hover sobre el texto "Las 6 Mentiras" = disipa todo
    float isHoveredText = 1.0 - step(0.02, length(uHoverTextUV.xy - vec2(0.1384, 0.7569)));
    float revealFactor = mix(1.0, 0.1, isHoveredText * uHoverTextAlpha);
    ...
    if (revealFactor > 0.01) {
        ...
        float rawAlpha = density * maskMentiras * revealFactor * edgeSoftness;
