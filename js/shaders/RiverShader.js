export const riverVertexCommon = `
#include <common>
varying vec2 vUv;
...
export const riverVertexUv = `
#include <uv_vertex>
vUv = uv;
vGlobalPos = position;
`;

export const riverFragmentCommon = `
#include <common>
varying vec2 vUv;
varying vec3 vGlobalPos;
uniform float uTime;
...
export const riverFragment = `
#include <dithering_fragment>
    
    // Leemos la textura empaquetada (R=Ríos, G=Lagos, B=Desierto, A=Nieve)
    vec4 packedMasks = texture2D(tPackedMasks, vUv);
    
    // El canal Rojo (r) contiene la máscara de los ríos (líneas negras sobre fondo blanco)
    float isRiver = 1.0 - smoothstep(0.1, 0.5, packedMasks.r);
    
    // Si no es un río, descartamos el píxel inmediatamente (100% transparente)
    if (isRiver < 0.01) {
        discard;
    }
